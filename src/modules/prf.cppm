// Own the primitive recursive functions. STL-only.
...
class Function {
 public:
  ...
  std::uint64_t operator()(const std::vector<std::uint64_t>& args, Trace& trace) const {
    ...
    std::uint64_t r = eval_(args, trace);
    scope.set_result(r);
    return r;
  }

 protected:
  virtual std::uint64_t eval_(const std::vector<std::uint64_t>& args, Trace& trace) const = 0;
};
...
class Successor : public Function {
 ...
 protected:
  std::uint64_t eval_(const std::vector<std::uint64_t>& args, Trace&) const override {
    if (args[0] == std::numeric_limits<std::uint64_t>::max()) {
      throw std::overflow_error("Successor overflow on std::uint64_t");
    }
    return args[0] + 1ULL;
  }
...
class Projection : public Function {
 public:
  Projection(std::size_t i, std::size_t n, std::string name = "") : i_(i), n_(n) {
    if (i_ == 0 || i_ > n_) {
 ...
 public:
  Composition(
    ...
    if (name.empty()) {
      std::ostringstream oss;
      oss << "C(" << g_->name() << "∘[";
      for (std::size_t i = 0; i < hs_.size(); ++i) {
        if (i)
          oss << ", ";
        oss << hs_[i]->name();
      }
...
class PrimitiveRecursion : public Function {
 ...
 protected:
  ...
  std::uint64_t eval_(const std::vector<std::uint64_t>& args, Trace& trace) const override {
    ...
    const std::uint64_t y = args.back();
    const std::size_t n_x = args.size() - 1;
    std::vector<std::uint64_t> xs(args.begin(), args.end() - 1);

    // Base case
    std::uint64_t acc = (*g_)(xs, trace);
    if (y == 0ULL)
      return acc;

    // Reuse a single buffer for h's arguments: (t, acc, x...)
    std::vector<std::uint64_t> h_args;
