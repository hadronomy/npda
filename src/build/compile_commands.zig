//! Original source: https://github.com/xxxbrian/zig-compile-commands
//! waiting for the PR to be merged:
//! https://github.com/ziglang/zig/pull/22012
...
pub fn createStep(b: *std.Build, name: []const u8, options: CompileCommandOptions) !void {
    ...
    var compile_steps_list: std.ArrayList(*std.Build.Step.Compile) = try .initCapacity(b.allocator, 1_000);
    if (options.target) |target| compile_steps_list.append(b.allocator, target) catch @panic("OOM");
    compile_steps_list.appendSlice(b.allocator, options.targets) catch @panic("OOM");
    ...
    while (index < compile_steps_list.items.len) {
        ...
        for (compile_step.root_module.link_objects.items) |link_object| {
            switch (link_object) {
                ...
                .other_step => {
                    compile_steps_list.append(b.allocator, link_object.other_step) catch @panic("OOM");
                },
...
pub fn extractIncludeDirsFromCompileStep(b: *std.Build, step: *std.Build.Step.Compile, include_emitted_include_trees: bool) ![]const []const u8 {
    var dirs: std.ArrayList([]const u8) = try .initCapacity(b.allocator, 1_000);
            ...
            .other_step => |compile| {
                for (compile.installed_headers.items) |install_header| {
                    switch (install_header) {
                        .file => {},
                        .directory => |dir| {
                            dirs.append(b.allocator, dir.source.getPath(compile.step.owner)) catch @panic("OOM");
                        },
                    }
                }

                if (include_emitted_include_trees) {
                    dirs.append(b.allocator, compile.getEmittedIncludeTree().getPath(compile.step.owner)) catch @panic("OOM");
                }
            },
            .path, .path_system, .path_after, .framework_path, .framework_path_system => |path| {
                dirs.append(b.allocator, path.getPath(b)) catch @panic("OOM");
            },
            // TODO: support this
            .config_header_step => {},
            .embed_path => {},
...
fn getCSources(b: *std.Build, options: CompileCommandOptions) ![]CSources {
    ...
    var res: std.ArrayList(CSources) = try .initCapacity(allocator, 1_000);

    // move the compile steps into a mutable dynamic array, so we can add
    // any child steps
    var compile_steps_list: std.ArrayList(*std.Build.Step.Compile) = try .initCapacity(allocator, 1_000);
    if (options.target) |target| compile_steps_list.append(allocator, target) catch @panic("OOM");
    compile_steps_list.appendSlice(allocator, options.targets) catch @panic("OOM");
    ...
    while (index < compile_steps_list.items.len) {
        ...
        var shared_flags: std.ArrayList([]const u8) = try .initCapacity(allocator, 1_000);
        ...
        for (try extractIncludeDirsFromCompileStep(b, compile, options.include_emitted_include_trees)) |include_dir| {
        ...
        for (compile.root_module.link_objects.items) |link_object| {
            switch (link_object) {
                ...
                .c_source_file => {
                    ...
                    var flags: std.ArrayList([]const u8) = try .initCapacity(allocator, 100);
                    ...
                    source_file.* = CSourceFiles{
                        ...
                        .language = .c, // Add this line to fix the missing field
                    };
                ...
                .c_source_files => {
                    ...
                    var flags: std.ArrayList([]const u8) = try .initCapacity(allocator, 1_000);
    ...
    return res.toOwnedSlice(allocator) catch @panic("OOM");
}

const Progress_Node = if (zig_version.major > 0 or zig_version.minor >= 13) std.Progress.Node else *std.Progress.Node;
// Updated the function signature to use MakeOptions instead of Progress.Node
fn makeCdb(step: *std.Build.Step, options: std.Build.Step.MakeOptions) anyerror!void {
    _ = options;
    const allocator = step.owner.allocator;

    var compile_commands: std.ArrayList(CompileCommandEntry) = try .initCapacity(allocator, 1_000);
    ...
    const c_sources = try getCSources(step.owner, the_options);
    ...
    for (c_sources) |sources| {
        ...
        for (sources.c_source_files.files) |c_file| {
            ...
            var arguments: std.ArrayList([]const u8) = try .initCapacity(allocator, 200);
            // pretend this is clang compiling
            arguments.append(allocator, "clang") catch @panic("OOM");
            arguments.append(allocator, c_file) catch @panic("OOM");
            arguments.appendSlice(allocator, &.{ "-o", std.fmt.allocPrint(allocator, "{s}.o", .{c_file}) catch @panic("OOM") }) catch @panic("OOM");
            arguments.appendSlice(allocator, flags) catch @panic("OOM");
            arguments.appendSlice(allocator, sources.compile.root_module.c_macros.items) catch @panic("OOM");
            ...
            //         try arguments.append(allocator, try common.includeFlag(allocator, include_dir));
            //     }
            // }
